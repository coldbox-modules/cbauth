		...
		describe( "authentication service", function(){
			...
			describe( "instantiating the user service", function(){
				...
				it( "caches the userService", function(){
					...
					expect( wireboxMock.$once( "getInstance" ) ).toBeTrue(
						"[getInstance] should only have been called once."
					);
				} );
			} );
			...
			describe( "helper functions", function(){
				...
				describe( "getUser", function(){
					...
					it( "returns the user from the request if it exists", function(){
						...
						expect( sessionStorageCallLog ).notToHaveKey(
							"exists",
							"[exists] should not be called on the SessionStorage"
						);
						expect( sessionStorageCallLog ).notToHaveKey(
							"get",
							"[get] should not be called on the SessionStorage"
						);
					} );
					...
					it( "caches the user from in request scope", function(){
						...
						expect( sessionStorageCallLog.exists ).toHaveLength(
							1,
							"[exists] should only have been called once."
						);
						expect( sessionStorageCallLog.get ).toHaveLength(
							1,
			...
			describe( "logging in", function(){
				it( "stores a user id in the session storage", function(){
					...
					expect( sessionStorageMock.$callLog().set[ 1 ][ 2 ] )
						// first time called, second argument
			...
			describe( "authenticating users", function(){
				...
				describe( "calling user-defined functions", function(){
					it( "calls a user-defined function to validate correct credentials", function(){
						...
						expect( userServiceMock.$once( "isValidCredentials" ) ).toBeTrue(
							"[isValidCredentials] was not called on the userService."
						);
						expect( userServiceMock.$once( "retrieveUserByUsername" ) ).toBeTrue(
							"[retrieveUserByUsername] was not called on the userService."
						);
					} );
					...
					it( "throws an InvalidCredentials exception and does not call retrieveUserByUsername if the credentials are invalid", function(){
						...
						expect( userServiceMock.$never( "retrieveUserByUsername" ) ).toBeTrue(
							"[retrieveUserByUsername] should not have been called on the userService but was."
						);
					} );
				} );
